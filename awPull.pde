...
void setup() { 
  ...
  background(50); 
  fill(200);
  output = createWriter("wx_data.txt");
  output.println("TimeStamp,Indoor_Channel,Indoor_Battery,Outdoor_Channel,Outdoor_Battery,Second_Outdoor_Channel,Second_Outdoor_Battery,Indoor_Temp,Indoor_Hum,Abs_Press,Rel_Press,Outdoor_Temp,Outdoor_Hum,Wind_Dir,Wind_Speed,Max_Daily_Gust,SolRad,Uv,Uvi,PM2_5,Hour_Rain,Event_Rain,Daily_Rain,Weekly_Rain,Monthly_Rain,Yearly_Rain");
} 
...
void draw() {
   if (nextUpdate < millis()){
       ...
       c = new Client(this, "192.168.24.121", 80);  // ipObserver
       c.write("GET /livedata.htm HTTP/1.0\n");  // "GET" live data page from 
       nextUpdate = millis() + 15000;  //set next sample time.
       ...
       if (newData == true) {   //If there is some new data lets do something with it.
            ...
            wx_e[15] = (StringUtils.substringBetween(lines[117], "value=\"","\" maxlength="));  //Max Daily Gust
            wx_e[16] = (StringUtils.substringBetween(lines[122], "value=\"","\" maxlength="));  //Solar Raiation
            wx_e[17] = (StringUtils.substringBetween(lines[127], "value=\"","\" maxlength="));  //UV
            wx_e[18] = (StringUtils.substringBetween(lines[132], "value=\"","\" maxlength="));  //UVI
            wx_e[19] = (StringUtils.substringBetween(lines[137], "value=\"","\" maxlength="));  //PM 2.5
            wx_e[20] = (StringUtils.substringBetween(lines[142], "value=\"","\" maxlength="));  //Hourly Rain
            wx_e[21] = (StringUtils.substringBetween(lines[147], "value=\"","\" maxlength="));  //Event Rain
            wx_e[22] = (StringUtils.substringBetween(lines[155], "value=\"","\" maxlength="));  //Daily Rain
            wx_e[23] = (StringUtils.substringBetween(lines[160], "value=\"","\" maxlength="));  //Weekly Rain
            wx_e[24] = (StringUtils.substringBetween(lines[166], "value=\"","\" maxlength="));  //Monthly Rain
            wx_e[25] = (StringUtils.substringBetween(lines[172], "value=\"","\" maxlength="));  //Yearly Temp          
            ...
            for (int i = 0; i < 25; i++) {
               outputString += wx_e[i] + ",";
                ...
                output.println("Fail to get data");   // If the read did not produce 243 then it was not a good read.  Display an error.
       }
